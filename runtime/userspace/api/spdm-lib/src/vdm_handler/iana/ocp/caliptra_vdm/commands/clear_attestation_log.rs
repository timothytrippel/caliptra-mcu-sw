...
#[repr(C)]
struct ClearAttestationLogReq {}

impl CommonCodec for ClearAttestationLogReq {}

pub(crate) async fn handle_clear_attestation_log(
    handler: &dyn CaliptraCmdHandler,
    _req_buf: &mut MessageBuf<'_>,
    rsp_buf: &mut MessageBuf<'_>,
) -> VdmResult<CaliptraVdmCmdResult> {
    match handler.clear_log(LogType::Attestation as u32).await {
        Ok(()) => {
