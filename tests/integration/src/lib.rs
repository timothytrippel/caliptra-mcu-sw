...
mod test {
    ...
    pub fn build_primary_flash_initial_contents(
        ...
        if let Some(cert) = ecc_cert {
            flash[otp_part.offset..otp_part.offset + cert.len()].copy_from_slice(cert);
        }
    ...
    pub fn run_imaginary_flash_controller_service(hw: &mut DefaultHwModel) {
        #[cfg(feature = "fpga_realtime")]
        {
            ...
            let external_otp_memory = ECC_DEVID_CERT_DER.to_vec();
            let primary_flash_initial_contents =
                build_primary_flash_initial_contents(None, Some(&external_otp_memory), None);
